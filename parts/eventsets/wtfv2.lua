...
ultraBone.draw=function(P,repMode)
    ...
    if UB~=1 then
        if h and not (D.hideHold and not (P.result=='win' or P.result=='lose' or repMode)) then
            ...
            ultraBone.drawActive(UB,h.bk,-5,-17) --hold
            if D.hideHold then gc_setColor(UBcolor) end
        elseif h then
            FONT.set(62,'mono')
            GC.mStr("?",62,0)
        end
        for i=1,P.gameEnv.nextCount do ultraBone.drawActive(UB,P.nextQueue[i].bk,11,3*i-20)end --next
    end
...
return {
    mesDisp=function(P)
			...
			gc_setColor(1,1,1,1)
			gc_draw(IMG.crash,-1350,-1550,0,3)
			end
		D=P.modeData
        if D.ultraBone then
            gc_push('transform')
	...
	love.audio.setVolume(SETTING.mainVol)
        ...
        while true do
			...
			if P.modeData.tCycle%20==0 then P.modeData.randomOffset=20 end --then P.modeData.randomOffset=math.random(2,7) end
			if P.modeData.UBflash==1 then UBcolor={0,1,0,1-math.ceil((P.modeData.tCycle%40)-P.modeData.randomOffset)} end
			
			if P.modeData.bMove==1 and P.modeData.tCycle%20==0 then 
			PLAYERS[1]:movePosition((340+(600-(600*P.modeData.bSize))/2),(60+(600-(600*P.modeData.bSize))/2),P.modeData.bSize) 
			end

			if P.modeData.s4line==1 and P.modeData.tCycle%20==0 and P.modeData.s4lineL~=10 then 
			P:garbageRise(17,1,P:getHolePos()) 
			P.modeData.s4lineL=P.modeData.s4lineL+1 
			end
			if P.modeData.blink==1 and P.modeData.tCycle%20==0 and P.modeData.blinkL<10 then
			...
			if P.modeData.sInv==1 and P.modeData.tCycle%40==30 then
			P.modeData.sInv1=0
			end
            ...
            while P.modeData.tCycle%20==0 and P.stat.frame>=warnTime[P.modeData.section] do
				...
				love.audio.setVolume(0)
								P.modeData.bSize=0.25
				PLAYERS[1]:movePosition((50+(600-(600*P.modeData.bSize))/2),(160+(600-(600*P.modeData.bSize))/2),P.modeData.bSize)
				end
